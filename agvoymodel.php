 ...
 * @return array
 */
function admin_get_all_circuits()
{
...
/**
 * Restreint la liste des circuits à ceux qui sont programmés
 * et renvoie la liste des circuits programmés
 * Mais malheureusement ne fontionne pas :(
 * 
 * @return array
 */
...
{
	global $list_of_circuits;
	$list_of_programmations = get_all_programmations();
	foreach ($list_of_circuits as $circuit) {
		if ( in_array($circuit, $list_of_programmations)){
			$programmed = array();
			global $programmed;
			$programmed[] = $circuit;
		};
